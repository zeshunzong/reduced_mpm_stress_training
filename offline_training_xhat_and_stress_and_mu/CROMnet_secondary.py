...
class CROMnet_secondary(pl.LightningModule):
    def __init__(self, data_format, preprop_params, example_input_array, initial_lr, batch_size, lr, epo, lbl, scale_mlp, train_every_n_points, ks, strides, siren_enc, siren_dec, enc_omega_0, dec_omega_0, verbose, loaded_from=None):
        super(CROMnet_secondary, self).__init__()
    ...
    def training_step(self, train_batch, batch_idx):
        # input("enter training step")
        encoder_input = train_batch['encoder_input']
        # print("in training step: encoder_input requires grad: ", encoder_input.requires_grad)
        # input()
        q = train_batch['q']
        outputs_local, _, _= self.forward(encoder_input) # outputs_local = decoder(xhat X)

        loss = 1000.0 * self.criterion(outputs_local, q)
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        mu = test_batch['mu'].detach().cpu().numpy().astype(float)
        ...
        for i in range(batch_size_local):
            ...
            sim_state = SimulationState(filename_out, False, input_x, input_q, input_t, input_mu, label=label)
    ...
    def forward(self, x):
        ...
        # x = (xhat, X, mu), shape is [batch_size, nparticles, lbl+input_dim]
        x = x
        batch_size_local = x.size(0)
        label = x[:,0, 0:self.lbllength] # shape = [lbl]     
        decoder_input = x.view(batch_size_local * x.size(1), x.size(2)) # shape = [batch_size*n_particle, lbl + input_dim]
        #decoder -> x
...
class NetDec(pl.LightningModule):
    def __init__(self, data_format, lbllength, scale_mlp, siren, omega_0):
        ...
        print("lbl: ", lbllength)
        print("idim:", data_format['i_dim'] )
        print("odim:", data_format['o_dim'] )
        print("mudim:", data_format['mu_dim'] )
        # print("decoder input dim: ", lbllength + data_format['i_dim'] + data_format['mu_dim'])
...
class Prepare(nn.Module):
    def __init__(self, lbllength, siren, data_format):
        ...
        self.siren = siren
        self.lbllength = lbllength
        self.i_dim = data_format['i_dim']
        self.o_dim = data_format['o_dim']
        self.mu_dim = data_format['mu_dim']
    ...
    def forward(self, x):
        ...
        x0 = x[:, self.lbllength:self.lbllength + self.i_dim]
        mu = x[:, self.lbllength + self.i_dim:]
        x0 = self.prep(x0)
        
        x = torch.cat((xhat, x0, mu), 1)
        return x
    ...
    def grad_func(self, x):
        ...
        with torch.no_grad():
            grad_mu = torch.Tensor([1]).type_as(x)
        grad_mu = grad_mu.expand_as(mu)

        grad_batch = torch.cat((grad_xhat, grad_x0, grad_mu), 1)
        grad_batch = torch.diag_embed(grad_batch)
