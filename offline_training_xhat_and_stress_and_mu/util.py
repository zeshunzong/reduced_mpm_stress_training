...
class DataList(object):
    def __init__(self, root_dir, train_ratio, train_every_k_timestep = 1):
        self.data_list, self.data_train_list, self.data_test_list, self.data_train_dir, self.data_test_dir = obtainFilesRecursively(
            root_dir, train_ratio, train_every_k_timestep)

def obtainFilesRecursively(path, train_ratio, train_every_k_timestep = 1):
    ...
    for dirname in dir_list_sim:
        ...
        for filename in os.listdir(os.path.join(path, dirname)):
            ...
            if file_number % train_every_k_timestep != 0:
                continue
            # skip files finish
            # print(file_number)
...
def convertInputFilenameIntoOutputFilename(filename_in, path_basename):
    ...
    pardirname = os.path.dirname(os.path.dirname(dirname))
    pardirname += '/pred'
